...
After block.'''
    ...
    def test_bad_backslash_escape_in_string_value(self):
        """Issue #120 — model emits a `write` arg whose `content` is Python
        source like `f'{x:05d}\\x1e'` (a single backslash hex escape, valid
        Python but invalid JSON since `\\x` is not in the JSON escape
        alphabet). strict=False does NOT widen the escape alphabet, only
        control chars, so json.loads raises `Invalid \\escape` and the whole
        fence would bleed into chat. The extractor must retry once with
        bad backslashes doubled."""
        # Construct the raw on-the-wire bytes: a `\x1e` literally inside
        # the JSON string value. Use a raw string so the test source itself
        # contains the single-backslash sequence the model emitted.
        text = (
            "```json\n"
            r'{"name": "write", "arguments": {"content": "a\x1eb", "filePath": "/tmp/foo.py"}}'
            "\n```"
        )
        payloads, clean = proxy.extract_tool_calls_and_text(
            text, available_tool_names=["write"]
        )
        self.assertEqual(len(payloads), 1, "bad-escape JSON must extract after retry")
        self.assertEqual(payloads[0]["name"], "write")
        # The repaired string has the literal backslash sequence preserved
        # (the model's intent: write `\x1e` as Python source).
        self.assertEqual(payloads[0]["arguments"]["content"], r"a\x1eb")
        self.assertEqual(payloads[0]["arguments"]["filePath"], "/tmp/foo.py")
        self.assertNotIn("```json", clean)

    def test_bad_backslash_escape_multiple_in_one_string(self):
        """Several distinct bad escapes (`\\x`, `\\a`, `\\v`) in the same
        string value all get repaired in a single retry."""
        text = (
            "```json\n"
            r'{"name": "write", "arguments": {"content": "x\x1e\a\v y", "filePath": "/a"}}'
            "\n```"
        )
        payloads, _ = proxy.extract_tool_calls_and_text(
            text, available_tool_names=["write"]
        ...
        self.assertEqual(len(payloads), 1)
        self.assertEqual(payloads[0]["arguments"]["content"], r"x\x1e\a\v y")

    def test_bad_backslash_repair_preserves_valid_escapes(self):
        """A string mixing valid escapes (`\\n`, `\\"`) with a bad one
        (`\\x1e`) must keep the valid escapes intact — newline stays a
        real newline, escaped quote stays a literal quote."""
        text = (
            "```json\n"
            r'{"name": "write", "arguments": {"content": "line1\nline2 \"q\" \x1e end", "filePath": "/a"}}'
            "\n```"
        )
        payloads, _ = proxy.extract_tool_calls_and_text(
            text, available_tool_names=["write"]
        ...
        self.assertEqual(len(payloads), 1)
        # \n decoded to real newline, \" decoded to literal quote,
        # \x1e preserved as a literal `\x1e` (4-char sequence).
        self.assertEqual(
            payloads[0]["arguments"]["content"],
            'line1\nline2 "q" \\x1e end',
        )

    def test_bad_backslash_other_decoder_errors_still_bleed(self):
        """A block that fails parsing for a reason OTHER than `Invalid
        \\escape` (e.g. structural — missing colon) must NOT get rewritten
        by the repair pass. It should bleed into clean_text as before."""
        text = (
            "```json\n"
        ...
        self.assertEqual(payloads, [])
        # The malformed block stays in clean_text — the leniency must not
        # mask structural errors that aren't bad escapes.
        self.assertIn("```json", clean)
