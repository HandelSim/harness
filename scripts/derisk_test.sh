...
# --- firewall allowlist ------------------------------------------------------
#
# Every harness container reads /etc/harness/allowlist. Generate a temp file
# and point docker compose at it via HARNESS_ALLOWLIST_PATH so the bind-mount
# in docker-compose.yml resolves to a real path. The allowlist is a superset
# of init-firewall.sh's expected verify-step probes (api.github.com etc.).

ALLOWLIST_FILE="$(mktemp -t harness-derisk.XXXXXX.allow)"
cat >"${ALLOWLIST_FILE}" <<'EOF'
# auto-generated by derisk_test.sh
github.com
api.github.com
codeload.github.com
