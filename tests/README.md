...
All test artifacts for the harness project live here. Production runtime
tooling stays under `scripts/`; anything sourced from this directory is
test-only.
...
| Path                       | What it is                                                                  |
| -------------------------- | --------------------------------------------------------------------------- |
| `INVENTORY.md`             | Flat list of every testable feature/behavior in the harness, with stable IDs (`F###`, `P###`, `A###`, ...). The source of truth that coverage maps against. |
| `COVERAGE.md`              | Map of every inventory ID to its test(s), with a status per row and quoted assertion evidence. Regenerated by Track J2 after Track D strengthening. (See that file for the legend and current stats — not duplicated here.) |
| `lib/`                     | Sourceable bash test toolkits. `test_helpers.sh` is the main one (`require_docker`, `test_section`, `test_generate_env`, `test_generate_mockupstream_override`, `test_wait_for_healthy`, `test_cleanup`, plus integration helpers). |
| `fixtures/`                | Test fixtures. `fixtures/responses/` holds mock-upstream response fixtures (see `fixtures/responses/README.md`); `fixtures/test-project/` is the small Python calculator package used by `integration_test.sh`. |
| `mock_upstream.py`         | Mock upstream LLM API used by every docker-based test. Supports a legacy `MOCK_SCENARIO=text\|tool` mode and the richer fixture-dispatch mode (`MOCK_FIXTURES_DIR=/fixtures`). |
| `*_test.sh`                | Top-level test scripts (one per area: `proxy`, `harness`, `persistence`, `mcp`, `firewall`, `upgrade`, `full_pipeline`, `integration`, `scheme_contract`, `podman_smoke`). |
| `e2e/`                     | tmux-driven TUI scenarios. `e2e/lib/` (driver), `e2e/run.sh` (orchestrator), `e2e/scenarios/*.yaml` (per-scenario specs). See `e2e/README.md`. |
| `benchmarks/`              | Harbor-based agent benchmarks (Terminal-Bench 2.0, SWE-bench Lite). Adapters under `benchmarks/adapters/`, schemes under `benchmarks/schemes/`, runners under `benchmarks/runners/`. Benchmarks NEVER run in CI. See `benchmarks/README.md`. |
...
newly-added `tests/<name>_test.sh` is picked up automatically — no wrapper
edits needed.
...
| Variable                  | Effect                                                                  |
| ------------------------- | ----------------------------------------------------------------------- |
| `HARNESS_RUN_SLOW`        | `=1` opts into the slow integration test (`integration_test.sh`). `harness test --slow` sets this for you. The default test suite stays fast without it. |
| `HARNESS_E2E_PATTERN`     | Glob restricting which scenarios `tests/e2e/run.sh` will execute (default: `tests/e2e/scenarios/*.yaml`). |
| `HARNESS_E2E_LOG_DIR`     | Where the tmux driver writes per-scenario `pipe-pane` transcripts. Defaults to a temp dir under the worktree. Set this to keep transcripts after a failed run. |
| `HARNESS_CONTAINER_RUNTIME` | `=podman` runs the suite under podman instead of docker. All `*_test.sh` honor this. |

The `mock_upstream.py` honors `MOCK_SCENARIO` (legacy) or `MOCK_FIXTURES_DIR`
(fixture dispatch). Tests set these directly; you do not normally set them
from your shell.
...
1. Drop a `tests/<name>_test.sh` file using `tests/lib/test_helpers.sh` for
   common setup. Use existing scripts (`proxy_test.sh`, `mcp_test.sh`) as
   the model — the `test_section` helper structures the output, and
   `test_cleanup` runs on EXIT.
...
3. If the test needs a mock upstream response that doesn't exist yet, add
   a fixture under `tests/fixtures/responses/` (see that directory's
...
5. If the test is slow (> ~60s) or needs a heavy image, gate it behind
...
- `harness benchmark terminal-bench` — full Terminal-Bench 2.0 run; 6-12 hrs.
...
and adding a new benchmark target.

## Test file conventions

- **Shebang and flags.** `#!/usr/bin/env bash` with `set -euo pipefail` at
  the top. Tests must fail loudly, not silently.
- **Source the toolkit.** `source "$(dirname "$0")/lib/test_helpers.sh"`.
  Use `require_docker` (or the equivalent) at the top so the test errors
  out cleanly when prereqs are missing rather than crashing mid-run.
- **Sectioned output.** `test_section "what this section verifies"` before
  each block of assertions. Makes failures easy to locate in CI logs.
- **Assert on structure, not noise.** Real assertions look like
  ...
  output, or `grep ... > /dev/null; true` with no exit check. Track C's
  audit flags these explicitly.
...
- **No real upstream.** Every docker-based test wires the agent to
  `tests/mock_upstream.py` via `test_generate_mockupstream_override` or an
  equivalent compose override. Tests must never hit a real LLM API.
- **CI-safe.** The default suite must run in under ~10 min total and use
  no more than ~7 GB RAM / ~14 GB disk per matrix shard. Heavy tests gate
  behind `HARNESS_RUN_SLOW=1`.
...
CI runs the following jobs on every push to `dev` and `main` and on every
PR targeting either branch (see `.github/workflows/ci.yml`):
