...
- `proxy/test_proxy.py` (5007 lines) — pure-Python unit tests for `format_tools_to_text`,
...
- `tests/upgrade_test.sh` (1175 lines, no docker) — `envfile_merge`, `linefile_merge`,
  `directory_overwrite`, `userfile_sync`, `_upgrade_confirm`, `_upg_can_prompt`,
  `_upgrade_select_actions`, synthetic N→N+1 upgrade, rsync fallback.
...
- `tests/unit_host_test.sh` (no docker) — containerless `harness host`
  ...
  jq guard), `host_proxy_fingerprint`, `host_python_bin` interpreter
  verification (Windows alias-stub rejection), and the stderr-preservation
  invariant of the two loopback `/dev/tcp` probes (T15, against a real
  throwaway listener). Covers Ho001–Ho007, Ho018, Ho019.
...
| P059 | green  | proxy/test_proxy.py:test_bullet_labels_are_the_current_set (+test_load_bearing_content_survives_the_bullet_split / test_mode_hybrid_reminder_bullet_order / test_mode_hybrid_reminder_advises_default_to_tools / test_mode_hybrid_reminder_advises_concurrent_task_agents / test_mode_hybrid_reminder_has_honesty_rules / test_mode_hybrid_reminder_has_environment_context / test_filesystem_claims_clause_unconditional / test_mode_hybrid_reminder_does_not_point_at_agent_tools / test_mode_hybrid_reminder_never_names_the_agent_tools_marker / test_mode_hybrid_reminder_demands_the_smallest_diff / test_mode_hybrid_reminder_plans_through_to_verified / test_amnesia_bullet_makes_the_agents_md_check_recurring / test_agency_bullet_names_the_trigger_and_the_replacement / test_no_tool_fits_is_not_an_excuse_to_hand_back_instructions / test_reminder_closes_with_the_end_of_turn_check / test_mode_hybrid_reminder_lives_after_user_request / test_mode_hybrid_reminder_injects_known_host_os / test_mode_hybrid_reminder_omits_host_os_when_unknown) | Asserts the eight labelled bullets and their order (Act, don't describe / Amnesia / Todo list / Use the tools / Call format / Smallest change / Honesty / Environment; `- Delegate:` is asserted gone, its content folded into the tools bullet), that the load-bearing content survived the split out of the old three-bullet form, the prefer-a-listed-tool examples, concurrent `task` agents, anti-fabrication, the unconditional claims-come-from-tool-results clause, that the reminder does NOT name `<<<BEGIN_AGENT_TOOLS>>>` (that block sits on the head message, which oldest-first truncation drops first, so the pointer went dead exactly when it mattered), the plan-through-to-verified and smallest-diff clauses, the recurring AGENTS.md visibility check, the agency bullet's named slip-trigger and its rewrite into a tool call, that the no-tool-fits fallback is qualified rather than removed, and the end-of-turn check closing the block AFTER the per-tool entries (asserted by position and by the block's last characters, since the placement is the point) together with the imperative restatements and the worked fallback call that end it, the host-OS parenthetical rendered when `_HOST_OS` is set and omitted when empty, and that the guidance sits OUTSIDE the `<<<BEGIN_USER_MESSAGE>>>` wrap. TestHostOsSetup covers `HARNESS_HOST_OS` parse/normalise/default. | |
...
| P064 | green  | proxy/test_proxy.py (TestReminderTemplateFile) | test_install_root_overrides_the_adjacent_default (`$INSTALL_ROOT/reminder.md` wins over `reminder.md` beside proxy.py); test_leading_comment_block_is_stripped / _only_a_leading_comment_is_stripped / _trailing_newlines_are_stripped cover the parse; test_shipped_default_loads_and_carries_every_token asserts the real `proxy/reminder.md` loads (`!= _REMINDER_FALLBACK`) and carries `{{CWD}}`/`{{ENVIRONMENT}}`/`{{TODOS}}`/`{{TOOL_ENTRIES}}` and deliberately NOT `{{HOST_OS}}` (substituted, but the shipped prose reaches the host OS through `{{ENVIRONMENT}}` instead, so pairing them would double the parenthetical); test_missing_file_falls_back_loudly / _directory_at_the_path_falls_back / _empty_and_comment_only_files_fall_back assert `_REMINDER_TEMPLATE == _REMINDER_FALLBACK`; test_fallback_keeps_the_tool_call_envelope asserts `"name"`/`"arguments"` survive in the fallback; test_tokens_are_substituted_into_the_rendered_reminder covers the substitution; test_a_template_predating_the_new_tokens_warns / test_a_template_carrying_both_tokens_is_quiet cover the `[!]` that names `{{ENVIRONMENT}}`/`{{TODOS}}` when a user's seeded copy predates them. | |
...
| P093 | green  | proxy/test_proxy.py (TestRunModeSetup, TestEnvironmentClause) | test_host_is_honoured / test_case_and_whitespace_normalised / test_unknown_value_means_container / test_unset_means_container cover `_setup_run_mode`'s parse and its container default; test_host_mode_never_claims_a_container / test_container_mode_keeps_the_container_facts assert the two clauses (host mode never says container and warns off assuming Linux; container mode keeps the mounted-cwd and reproducibility facts); test_both_modes_name_a_known_host_os_inline / test_unknown_host_os_drops_the_parenthetical_in_both_modes cover the host-OS weave; test_reminder_renders_the_mode_specific_clause asserts `{{ENVIRONMENT}}` reaches the rendered reminder; test_host_os_token_still_substitutes_for_older_user_copies asserts a `reminder.md` carrying only the pre-`{{ENVIRONMENT}}` token still renders. | |
| P094 | green  | proxy/test_proxy.py (TestTodoReplay) | test_no_todowrite_in_history_yields_none / test_last_todowrite_wins / test_object_shaped_arguments_are_accepted / test_unparsable_arguments_are_skipped_not_raised / test_explicit_empty_list_beats_an_earlier_one cover `_extract_latest_todos`; test_present_list_renders_with_status_markers / test_unknown_status_normalises_to_pending / test_non_dict_and_empty_items_are_dropped / test_long_content_is_truncated / test_newlines_in_content_do_not_break_the_line_layout / test_structural_markers_in_content_are_defanged cover `_format_todos_block`'s rendering; test_over_cap_drops_finished_items_first / test_over_cap_with_nothing_finished_drops_the_TAIL / test_over_cap_reports_both_counts_separately / test_in_progress_survives_the_cap cover the cap policy (the TAIL test is the regression guard: dropping from the head deleted the model's next steps and reported them as finished); test_a_token_inside_a_todo_is_not_expanded asserts the single-pass substitution; test_absent_list_renders_the_create_one_instruction asserts the no-list prompt; test_reminder_carries_the_live_list / test_reminder_prompts_for_a_list_when_there_is_none cover the builder, and test_hybrid_translation_replays_the_list_end_to_end / test_hybrid_translation_prompts_when_history_has_no_list cover the full path through `translate_history_and_apply_prompt`. | |
...
| U030 | green  | tests/upgrade_test.sh:902-1021 (T13)        | T13 copies the shipped file over the user's, asserts `upgrade_userfile_needs_sync` returns non-zero and `jq -r .reason == "identical"`, and that no `.bak` was written. | |
| U031 | green  | tests/upgrade_test.sh:902-1021 (T13)        | T13 feeds `n` and then a bare Enter to `upgrade_userfile_sync` (via `HARNESS_CONFIRM_FROM_STDIN=1` and the real `_upgrade_confirm` sourced from `harness`): both give `reason == "declined"`, `MY OWN WORDING` still greps in the target, and no `.bak` exists. The bare Enter now gets there by exhausting the `require` re-asks rather than by defaulting on the first read. | The printed line delta is not asserted, only the decision. |
| U032 | green  | tests/upgrade_test.sh:902-1021 (T13)        | T13 feeds `y`: `files_updated[0] == "user.md"`, the target now `diff`s clean against the shipped source, and `user.md.bak` still greps `MY OWN WORDING`. | The `backup_failed` branch (unwritable directory) is not exercised. |
| U034 | green  | tests/upgrade_test.sh:1022-1146 (T14/T14b)  | T14 allocates a real pty with `script` (NO `HARNESS_CONFIRM_FROM_STDIN` — the hook is what hid the bug) and runs the real `apply_upgrade_actions` over a one-action manifest answering `y`: the target must now carry the shipped wording and `<target>.bak` the user's. T14b asserts `_upg_can_prompt` is true inside a loop whose FD 3 is redirected, and false under `setsid` (no controlling terminal). Skips cleanly without `script`, host `jq`, or `setsid`. | The pty half is skipped where `script` is unavailable; T14b still runs. |
| U035 | green  | tests/upgrade_test.sh:1147-1228 (T15)       | T15 drives `_upgrade_select_actions` over a 4-action manifest (2 merges, 1 `userfile_sync`, 1 registry action) via `HARNESS_CONFIRM_FROM_STDIN=1` — faithful now that the loop reads candidates on FD 3 — and asserts the accepted subset for `y,y,y`, `n,y,y` (the point of the change: one merge without the other), `y,n,n`, three bare Enters, a lone `a`, a lone `q` (empty, including the `userfile_sync` id), and an unrecognized answer (declines). | |
| U036 | green  | tests/upgrade_test.sh:1090-1117 (T14c)      | T14c reruns the T14 pty scenario against a second install root and feeds a blank line before the `y`: the target must still carry the shipped wording and `.bak` the user's. The shipped `_upgrade_confirm` returns 1 for `\ny` with default `n`, i.e. exactly the reported "I typed y and it kept my file". | Emulates the terminal's stray line with a blank one; the real Windows byte sequence is unknown. |
| U033 | green  | tests/upgrade_test.sh:902-1021 (T13)        | T13 asserts `reason` is `dry_run` with `dry_run=1`, `not_prompted` with `allow_prompt=0`, `target_missing` after deleting the target (and that nothing is created), and `source_missing` after deleting the source (with the user's file undamaged); `upgrade_userfile_needs_sync` returns non-zero for both missing cases and zero only when both exist and differ. Also asserts `upgrade_actions.sh` defines its own `_upgrade_confirm` fallback when sourced standalone. | The no-tty half of the not-prompted condition is covered via `allow_prompt=0`, not by detaching a tty. |
...
| Ho019 | green  | tests/unit_host_test.sh (T15)               | A throwaway python listener on an ephemeral loopback port; `host_proxy_wait_ready` (live pidfile + that port) and `host_mcp_port_open` (meta `host_port` = that port) each run in a subshell that echoes to stderr afterwards — the echo must be captured. Fails against the pre-fix `exec 3>&- 3<&- 2>/dev/null`. | |
