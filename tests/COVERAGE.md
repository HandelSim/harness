...
| P059 | green  | proxy/test_proxy.py:test_bullet_labels_are_the_current_set (+test_load_bearing_content_survives_the_bullet_split / test_mode_hybrid_reminder_bullet_order / test_mode_hybrid_reminder_advises_default_to_tools / test_mode_hybrid_reminder_advises_concurrent_task_agents / test_mode_hybrid_reminder_has_honesty_rules / test_mode_hybrid_reminder_has_environment_context / test_filesystem_claims_clause_unconditional / test_mode_hybrid_reminder_points_at_agent_tools / test_mode_hybrid_reminder_lives_after_user_request / test_mode_hybrid_reminder_injects_known_host_os / test_mode_hybrid_reminder_omits_host_os_when_unknown) | Asserts the eight labelled bullets and their order (Amnesia / Act, don't describe / Use the tools / Call format / Todo list / Delegate / Honesty / Environment), that the load-bearing content survived the split out of the old three-bullet form, the prefer-a-listed-tool examples, concurrent `task` agents, anti-fabrication, the unconditional claims-come-from-tool-results clause, the pointer at the `<<<BEGIN_AGENT_TOOLS>>>` section, the host-OS parenthetical rendered when `_HOST_OS` is set and omitted when empty, and that the guidance sits OUTSIDE the `<<<BEGIN_USER_MESSAGE>>>` wrap. TestHostOsSetup covers `HARNESS_HOST_OS` parse/normalise/default. | |
