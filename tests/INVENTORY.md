...
| F154 | `seed_tool_guidance_file` (called by `cmd_start` and `host_proxy_start`, a wrapper over the shared `seed_user_data_file` that also backs `seed_reminder_file`) copies the tracked `proxy/tool-guidance.json` to the gitignored `<install_root>/.harness-tool-guidance.json` when it is missing, never overwriting an edited copy, and `rmdir`s an empty directory docker left at that path. `cmd_start` then exports `HARNESS_TOOL_GUIDANCE_PATH` at the user copy; `host_proxy_start` passes it in the `nohup` env; the compose default stays the tracked file |
