...
| P059 | The hybrid recency reminder carries the current labelled bullet set (Act, don't describe / Amnesia / Todo list / Use the tools / Call format / Smallest change / Honesty / Environment) in that order — agency first, since that is the failure the block exists to stop, and the wording of every bullet is kept terse so the block does not dilute it: the model is told its history is silently truncated and that it must `read` AGENTS.md if it can no longer see one; that it acts rather than advises, and that announcing a step without the call in the same message is that same failure; that every claim about the working directory must come from a tool result and a listed tool beats doing the work by hand (`read`/`edit`/`write` over cat/sed, `grep`/`glob` over find, `webfetch` over curl); the one-complete-```json-block call format; that a detailed todo list is kept current AND planned through to verified (build/run/test steps in the list, run by the model rather than handed back); concurrent `task` agents (folded into the tools bullet, no longer its own label); that the diff stays as small as the request allows because a human reviews it; anti-fabrication; and the environment clause. The AGENTS.md rule is a per-turn visibility check, not a one-time step — a checked-off todo item is explicitly not evidence the file is still in context. The block CLOSES, after the per-tool entries, with an end-of-turn check: a turn ends either with a tool call or with the final report on finished work, and a turn carrying neither stops the opencode run outright. The closer then restates those rules imperatively (read AGENTS.md now, do the work rather than hand back instructions, verify rather than assume) and ends with a worked `bash` call for the model to emit if it doubts its tools work at all. The guidance sits AFTER the user's request, outside the `<<<BEGIN_USER_REQUEST>>>` wrap |
...
| P093 | The reminder's `{{ENVIRONMENT}}` clause tracks the run mode: `_setup_run_mode` reads `HARNESS_RUN_MODE`, and only `harness host` sets it (to `host`) — unset, blank, or any unrecognised value falls back to `container`, which is what every deployment did before the variable existed. Host mode says the agent runs directly on the user's own machine, must not assume Linux, and should keep installs project-local; container mode says Linux container with the cwd mounted, plus the reproducibility caveat. `{{HOST_OS}}` still substitutes on its own, because seeding never overwrites a user's existing `reminder.md`, so a copy predating `{{ENVIRONMENT}}` keeps working |
| P094 | The reminder replays the model's own todo list: `_extract_latest_todos` rescans the RAW inbound history for the LAST `todowrite` tool call (arguments as dict or JSON string; unparsable/non-list arguments skipped; an explicit empty list wins over an earlier non-empty one) and `_format_todos_block` renders it under the Todo list bullet with status markers (`[ ]`/`[~]`/`[x]`/`[-]`; a status outside opencode's four normalises to `pending`), content truncated at 140 chars, newlines flattened and `<<<` defanged, at most 15 items: finished ones are dropped first, then the TAIL, never the head (the head is the model's next steps), and the two counts are reported separately (`+N finished`, `+N later`) so a pending step is never labelled finished. No cache is involved — opencode re-sends the full history every turn, so the rescan is stateless and correct even after the upstream has dropped every earlier turn. With no list at all the block instead orders the model to call `todowrite` before doing anything else |
